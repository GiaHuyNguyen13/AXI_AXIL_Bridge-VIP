class axi_axil_adapter_rd_ref_model;
 ...
 // Internal state variables to track burst processing
  ...
  function void axi_read_request(input axi_item axi_tx);
    this.axi_burst_len = axi_tx.arlen;  // AXI burst length
    this.axi_burst_size = axi_tx.arsize;  // AXI burst size
    this.axi_addr = axi_tx.araddr;  // Starting address
  endfunction
...
function axil_item convert_axi_to_axil();
  ...
  axil_tx.araddr = this.axi_addr;  // Pass the address to AXI-Lite

  // Fetch data from the centralized memory model
  axil_tx.rdata = this.mem.read(this.axi_addr);  // Use the centralized memory for read data
  ...
  axil_tx.arprot = 3'b000;  // Assuming no special protection bits
  axil_tx.arvalid = 1'b1;   // Valid read request
  axil_tx.rvalid = 1'b1;    // Valid read response
  axil_tx.rresp = 2'b00;    // OKAY response

  return axil_tx;
endfunction

  // Task to convert an entire AXI burst into AXI-Lite transactions
  task convert_burst_to_axil(output axil_item axil_tx[]);
    int idx = 0;
